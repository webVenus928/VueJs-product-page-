<template>
  <div class="cmn-option-style" id="serviceTime">
    <h3 class="primary-title">
      Would you like to add extended warranty coverage?.
    </h3>
    <a
      v-for="(item, index) in warrentyData"
      :key="index"
      :href="`#${item.id}`"
...
export default {
  ...
  methods: {
    setSeletedItem(item) {
      this.$store.commit("SET_WARRENTY", item);
      this.seleted = item;
    },
  },
  computed: {
    warrenty() {
      return this.$store.getters.warrenty;
    },
  },
  created() {
    this.setSeletedItem(this.warrentyData[0]);
  },
};
</script>
